...
import { Search, Filter, Download, UserCheck, Clock, CheckCircle, XCircle, Store } from "lucide-react";
import { exportToCSV } from "@/utils/export";

interface GlobalAttendanceRecord {
  id: number;
  employeeName: string;
  vendorName: string;
  role: string;
...
export default function SuperAdminAttendancePage() {
  const [records, setRecords] = useState<GlobalAttendanceRecord[]>([
    { id: 1, employeeName: "Arif Hossain", vendorName: "Tech Hub BD", role: "Sales Executive", date: "2023-10-26", in: "09:05 AM", out: "06:10 PM", duration: "9h 5m", status: "Present" },
    { id: 2, employeeName: "Jahid Hasan", vendorName: "Tech Hub BD", role: "Inventory Manager", date: "2023-10-26", in: "08:55 AM", out: "06:00 PM", duration: "9h 5m", status: "Present" },
    { id: 3, employeeName: "Sarah Ahmed", vendorName: "Smart Electronics", role: "Store Manager", date: "2023-10-26", in: "10:15 AM", out: "04:30 PM", duration: "6h 15m", status: "Half Day" },
    { id: 4, employeeName: "Kamrul Islam", vendorName: "Smart Electronics", role: "Delivery Staff", date: "2023-10-26", in: "08:00 AM", out: null, duration: null, status: "In Progress" },
    { id: 5, employeeName: "Nasir Uddin", vendorName: "Gadget Zone", role: "Sales Executive", date: "2023-10-26", in: null, out: null, duration: null, status: "Absent" },
  ]);
  ...
  const filteredRecords = records.filter(rec => 
    ...
    rec.vendorName.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const handleExport = () => {
    exportToCSV(
      filteredRecords, 
      ["Employee Name", "Vendor", "Role", "Date", "Check In", "Check Out", "Duration", "Status"], 
      "Global_Staff_Attendance_Export", 
      (item: GlobalAttendanceRecord) => [item.employeeName, item.vendorName, item.role, item.date, item.in || "-", item.out || "-", item.duration || "-", item.status]
    );
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Global Attendance</h2>
          <p className="text-slate-500 text-sm mt-1 font-medium tracking-tight">System-wide staff attendance monitoring.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <h3 className="text-2xl font-black text-slate-800">384</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm flex items-center gap-4">
          <div className="w-14 h-14 rounded-2xl bg-purple-50 flex items-center justify-center text-purple-500"><Store size={24} /></div>
          <div>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Active Vendors</p>
            <h3 className="text-2xl font-black text-slate-800">28</h3>
          </div>
        </div>
        <div className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm flex items-center gap-4">
      ...
      <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
        <div className="p-8 border-b border-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/20">
          <div className="relative max-w-md w-full">
            ...
            <input 
              type="text" 
              placeholder="Search by name, vendor, or role..." 
              value={searchQuery}
          ...
          <div className="flex gap-3">
            ...
            <button className="p-3 bg-white border border-slate-100 rounded-xl text-slate-400 hover:text-primary transition-all shadow-sm flex items-center gap-2">
              <Store size={16} /> <span className="text-xs font-bold hidden sm:inline">All Vendors</span>
            </button>
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-slate-50/30">
                ...
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">Vendor Org</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredRecords.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/40 transition-colors group">
                  ...
                  <td className="px-8 py-5">
                    <span className="text-xs font-bold text-slate-600 bg-slate-100 px-2.5 py-1 rounded-md">{item.vendorName}</span>
                  </td>
