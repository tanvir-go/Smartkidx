...
export default function EmployeeCustomerNotesPage() {
  const [data, setData] = useState([
  {
    "id": 1,
    "customer": "John Smith",
    "note": "Interested in wholesale prices.",
    "date": "2023-10-20"
  }
]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<any>(null);
  const [formData, setFormData] = useState({"customer":"","note":"","date":""});
  ...
  const handleOpenModal = (item: any = null) => {
    if (item) {
      ...
      setEditingItem(null);
      setFormData({"customer":"","note":"","date":""});
    }
    setIsModalOpen(true);
  };
  ...
  const handleExport = () => {
    exportToCSV(data, ["Customer","Note","Date"], "Export", (item: any) => [item.customer, item.note, item.date]);
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Customer Notes</h2>
          <p className="text-slate-500 text-sm mt-1 font-medium tracking-tight">CRM interaction logs.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/30">
                ...
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">Note</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {data.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/40 transition-colors">
                  ...
                  <td className="px-8 py-6 text-sm font-bold text-slate-700">{item.note}</td>
            ...
            <div className="flex-1 overflow-y-auto p-12 custom-scrollbar">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <input type="text" value={formData.note} onChange={e => setFormData({...formData, note: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-4 focus:ring-primary/5 outline-none" required />
                </div>
                <div className="space-y-2">
