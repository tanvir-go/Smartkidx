...
export default function EmployeeCreateOrdersPage() {
  ...
  const [formData, setFormData] = useState({"customer":"","items":"1","status":"Draft"});
  ...
  const handleOpenModal = (item: any = null) => {
    if (item) {
      ...
      setEditingItem(null);
      setFormData({"customer":"","items":"1","status":"Draft"});
    }
    setIsModalOpen(true);
  };
  ...
  const handleExport = () => {
    exportToCSV(data, ["Customer","Items","Status"], "Export", (item: any) => [item.customer, item.items, item.status]);
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1 font-medium tracking-tight">Manual order entry portal.</p>
        </div>
