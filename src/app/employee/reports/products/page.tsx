...
export default function EmployeeProductPerformancePage() {
  const [data, setData] = useState([
  {
    "id": 1,
    "product": "Robotics Kit V1",
    "sold": "342",
    "rev": "৳ 855,000"
  }
]);
  ...
  const [formData, setFormData] = useState({"product":"","sold":"","rev":""});
  ...
  const handleExport = () => {
    exportToCSV(data, ["Product","Units Sold","Revenue"], "Export", (item: any) => [item.product, item.sold, item.rev]);
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Product Performance</h2>
          <p className="text-slate-500 text-sm mt-1 font-medium tracking-tight">Top selling items analysis.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/30">
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">Product</th>
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">Units Sold</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {data.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/40 transition-colors">
                  <td className="px-8 py-6 text-sm font-bold text-slate-700">{item.product}</td>
                  <td className="px-8 py-6 text-sm font-bold text-slate-700">{item.sold}</td>
                  <td className="px-8 py-6 text-sm font-bold text-slate-700">{item.rev}</td>
            ...
            <div className="flex-1 overflow-y-auto p-12 custom-scrollbar">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <input type="text" value={formData.product} onChange={e => setFormData({...formData, product: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-4 focus:ring-primary/5 outline-none" required />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Units Sold</label>
                  <input type="text" value={formData.sold} onChange={e => setFormData({...formData, sold: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-4 focus:ring-primary/5 outline-none" required />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Revenue Generated</label>
                  <input type="text" value={formData.rev} onChange={e => setFormData({...formData, rev: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-4 focus:ring-primary/5 outline-none" required />
                </div>
