...
export default function EmployeeInventoryRequestsPage() {
  ...
  const [formData, setFormData] = useState({"reqId":"","item":"","qty":"","status":""});
  ...
  const handleOpenModal = (item: any = null) => {
    if (item) {
      ...
      setEditingItem(null);
      setFormData({"reqId":"","item":"","qty":"","status":""});
    }
    setIsModalOpen(true);
  };
  ...
  const handleExport = () => {
    exportToCSV(data, ["Request ID","Item","Qty","Status"], "Export", (item: any) => [item.reqId, item.item, item.qty, item.status]);
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1 font-medium tracking-tight">Internal stock requisitions.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/30">
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">Request ID</th>
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">Item</th>
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">Qty</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {data.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/40 transition-colors">
                  <td className="px-8 py-6 text-sm font-bold text-slate-700">{item.reqId}</td>
                  <td className="px-8 py-6 text-sm font-bold text-slate-700">{item.item}</td>
                  <td className="px-8 py-6 text-sm font-bold text-slate-700">{item.qty}</td>
            ...
            <div className="flex-1 overflow-y-auto p-12 custom-scrollbar">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Request ID</label>
                  <input type="text" value={formData.reqId} onChange={e => setFormData({...formData, reqId: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-4 focus:ring-primary/5 outline-none" required />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Item Name</label>
                  <input type="text" value={formData.item} onChange={e => setFormData({...formData, item: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-4 focus:ring-primary/5 outline-none" required />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Quantity</label>
                  <input type="text" value={formData.qty} onChange={e => setFormData({...formData, qty: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-4 focus:ring-primary/5 outline-none" required />
                </div>
                <div className="space-y-2">
