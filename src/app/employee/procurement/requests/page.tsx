...
export default function EmployeePurchaseRequestsPage() {
  ...
  const [formData, setFormData] = useState({"prId":"","dept":"","cost":"","status":""});
  ...
  const handleOpenModal = (item: any = null) => {
    if (item) {
      ...
      setEditingItem(null);
      setFormData({"prId":"","dept":"","cost":"","status":""});
    }
    setIsModalOpen(true);
  };
  ...
  const handleExport = () => {
    exportToCSV(data, ["PR ID","Department","Est. Cost","Status"], "Export", (item: any) => [item.prId, item.dept, item.cost, item.status]);
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Purchase Requests</h2>
          <p className="text-slate-500 text-sm mt-1 font-medium tracking-tight">Sourcing new inventory.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/30">
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">PR ID</th>
                ...
                <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50">Est. Cost</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {data.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50/40 transition-colors">
                  <td className="px-8 py-6 text-sm font-bold text-slate-700">{item.prId}</td>
                  <td className="px-8 py-6 text-sm font-bold text-slate-700">{item.dept}</td>
                  <td className="px-8 py-6 text-sm font-bold text-slate-700">{item.cost}</td>
            ...
            <div className="flex-1 overflow-y-auto p-12 custom-scrollbar">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">PR ID</label>
                  <input type="text" value={formData.prId} onChange={e => setFormData({...formData, prId: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-4 focus:ring-primary/5 outline-none" required />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Department</label>
                  <input type="text" value={formData.dept} onChange={e => setFormData({...formData, dept: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-4 focus:ring-primary/5 outline-none" required />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Est. Cost</label>
                  <input type="text" value={formData.cost} onChange={e => setFormData({...formData, cost: e.target.value})} className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl text-sm font-bold focus:ring-4 focus:ring-primary/5 outline-none" required />
                </div>
                <div className="space-y-2">
